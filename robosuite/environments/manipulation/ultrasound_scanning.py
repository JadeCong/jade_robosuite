"""
Author: Jade Cong
Date: 2021-07-05 00:06:44
LastEditTime: 2021-07-07 17:39:51
LastEditors: Jade Cong
Description: Definition of Ultrasound Scanning Env.
FilePath: /Workflows/Doing/PycharmProjects/aisono-robosuite/robosuite/environments/manipulation/ultrasound_scanning.py
"""
...
from robosuite.models.arenas import ScanArena
from robosuite.models.objects import SoftBreastObject, SoftFemaleTorsoObject, SoftHumanTorsoObject
from robosuite.models.tasks import ManipulationTask
from robosuite.models.tasks import UltrasoundScanningTask
from robosuite.utils.placement_samplers import UniformRandomSampler
from robosuite.utils.observables import Observable, sensor

import robosuite.utils.transform_utils as T
from robosuite.models.base import MujocoModel
from robosuite.utils.quaternion import distance_quat, difference_quat


class UltrasoundScanning(SingleArmEnv):
    """
    This class corresponds to the ultrasound scanning task for a single robot arm.
    ...
    Args:
        ...
        bed_full_size (3-tuple): x, y, and z dimensions of the bed.

        bed_friction (3-tuple): the three mujoco friction parameters for the bed.

        use_camera_obs (bool): if True, every observation includes rendered image(s)

        use_object_obs (bool): if True, include object (breast or torso) information in the observation.
    ...
    def __init__(
        ...
        bed_full_size=(0.8, 0.8, 0.05),
        bed_friction=(1., 5e-3, 1e-4),
        ...
        use_primitive_soft_object=True,
    ):
        assert robots == "Aubo" or robots == "Panda", "Robot must be Aubo or Panda!"
        # assert gripper_types == "UltrasoundProbeGripper", "Please try to specify UltrasoundProbeGripper!"
        assert "OSC" or "HMFC" in controller_configs["type"], "The robot controller must be of type OSC or HMFC!"

        # settings for bed top
        self.bed_full_size = bed_full_size
        self.bed_friction = bed_friction
        self.bed_offset = np.array((0, 0, 0.125))
        ...
        self.top_torso_offset = 0.039 if use_primitive_soft_object else 0.041  # offset from z_center of torso to top of torso
        self.x_range = 0.15  # how large the torso is from center to end in x-direction
        self.y_range = 0.09 if use_primitive_soft_object else 0.05  # how large the torso is from center to end in y-direction
        ...
        self.use_primitive_soft_object = use_primitive_soft_object
    ...
    def reward(self, action=None):
        ...
        # TODO: Need to redefine the different kinds of rewards.
        Reward: position, orientation, velocity, force and derivative force
        ...
        # get the reward about orientation
        ...
        # get the reward about derivative force
    ...
    def _load_model(self):
        ...
        # xpos = self.robots[0].robot_model.base_xpos_offset["scan"](self.bed_full_size[0])
        # self.robots[0].robot_model.set_base_xpos(xpos)

        # load model for table top workspace
        mujoco_arena = ScanArena(
            bed_full_size=self.bed_full_size,
        ...
        self.torso = SoftBreastObject(name="breast") if self.use_primitive_soft_object else SoftFemaleTorsoObject(name="torso")
        ...
        if self.placement_initializer is not None:
            ...
            self.placement_initializer = UniformRandomSampler(
                ...
                reference_pos=self.bed_offset,
                z_offset=0.01,
            )

        # task includes arena, robot, and objects of interest
        self.model = UltrasoundScanningTask(
    ...
    def visualize(self, vis_settings):
        """
        In addition to super call, visualize gripper site proportional to the distance to the torso.
        ...
        #     self._visualize_gripper_to_target(gripper=self.robots[0].gripper, target=self.torso)
    ...
    def _get_contacts_objects(self, model):
        ...
        assert isinstance(model, MujocoModel), "Inputted model must be of type MujocoModel; got type {} instead!".format(type(model))
        contact_set = set()
    ...
    def _check_probe_contact_with_bed(self):
        """
        Check if the probe is in contact with the bedtop.

        Returns:
            bool: True if probe is in contact with bed
        """
        return self.check_contact(self.robots[0].gripper, "geom_bed_mat")
